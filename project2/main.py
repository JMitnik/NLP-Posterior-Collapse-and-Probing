...
# %%
# Utility functions
get_pos_from_sent = lambda sent: [word['upostag'] for word in sent]
get_tokens_from_sent = lambda sent: [word['form'] for word in sent]
...
def fetch_sen_repr_transformer(corpus: List[TokenList], model, tokenizer) -> Tensor:
    ...
    for sent in corpus:
        ...
        # TODO: Do we get index 0 (`last_hidden_state`) or index 3 (`hidden_states`)?
        h_states = out[0]
        test_states = out[2]

        # Form output per token by averaging all subwords
        starting_idx = 0
        result = []
        for nr_tokens in subwords_per_token:
            subwords_per_token_tensor = h_states[starting_idx:starting_idx+nr_tokens, :]
            print(subwords_per_token_tensor[:, 0])
...
#     return pad_sequence(corpus_result).squeeze()


# %%
# Initialize the samples
sample_corpus = corpus = parse_corpus('data/sample/en_ewt-ud-train.conllu')
...
def fetch_sen_reps(ud_parses: List[TokenList], model, tokenizer) -> Tensor:
    ...
    return fetch_sen_repr_transformer(ud_parses, model, tokenizer)[0]
